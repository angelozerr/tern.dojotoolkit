...
(function() {
  ...
  function visitApi(dojoApi, ternDef, options) {
    for ( var name in dojoApi) {
      ...
      var ternItem = getTernItem(name, ternDef);
      if (dojoApi.superclass) {
        ternItem["!proto"] = dojoApi.superclass;
      }
      if (dojoItem.methods) {
  ...
  function getTernItem(name, ternDef) {
    var paths = name.split(/[.]/), ternParentItem = (name.indexOf('.') == -1) ? ternDef
        : ternDef["!define"], ternItem = null;
    for (var i = 0; i < paths.length; i++) {
      var path = paths[i], ternItem = ternParentItem[path];
      if (!ternItem) {
        ternItem = {};
  ...
  function visitMethods(dojoItem, ternItem) {
    for (var i = 0; i < dojoItem.methods.length; i++) {
      ...
      if (dojoMethod.name && !dojoMethod["private"]) {
        var ternMethod = ternItem[dojoMethod.name] = {};
        var type = 'fn(';
        type += ')';
        ternMethod["!type"] = type;
        if (dojoMethod.summary) ternMethod["!doc"] = dojoMethod.summary;
      }
    }
  }
  ...
  function toTernType(yuiType, dojoApi, isReturn, props) {
    if (!yuiType || yuiType === '*') {
      return '?';
    }
    // ex : Node|String
    ...
    default:
      return formatType(getClassName(yuiType, dojoApi), isArray, isReturn);
    }
  }
