(function(root, mod) {
  ...
  function getTernItem(name, ternDef) {
    var paths = name.split(/[.]/), ternParentItem = ternDef, ternItem = null;
  ...
  function populateMethod(dojoMethod, ternMethod) {
    ...
    type += ')';
    if (dojoMethod["return-types"]) {
      var returnTypes = dojoMethod["return-types"];
      for (var j = 0; j < returnTypes.length; j++) {
        var returnType = returnTypes[j].type;
  ...
  function getTernType(dojoType, dojoApi, isReturn) {
    ...
    } else if (dojoType.toLowerCase() === 'object') {
      return '?';
    }else {
      // ex : String||Object
